...
Item {
    ...
    property var allCommands: [
        {
            name: "attach",
            description: Translation.translate("Attach a file. Only works with Gemini."),
            execute: args => {
        ...
        {
            name: "model",
            description: Translation.translate("Choose model"),
            execute: args => {
                Ai.setModel(args[0]);
            }
        },
        {
            name: "tool",
            description: Translation.translate("Set the tool to use for the model."),
            execute: args => {
                // console.log(args)
                if (args.length == 0 || args[0] == "get") {
                    Ai.addMessage(Translation.translate("Usage: %1tool TOOL_NAME").arg(root.commandPrefix), Ai.interfaceRole);
                } else {
                    const tool = args[0];
                    const switched = Ai.setTool(tool);
                    if (switched) {
                        Ai.addMessage(Translation.translate("Tool set to: %1").arg(tool), Ai.interfaceRole);
                    }
                }
            }
        },
        {
            name: "prompt",
            description: Translation.translate("Set the system prompt for the model."),
            execute: args => {
                if (args.length === 0 || args[0] === "get") {
                    Ai.printPrompt();
                    return;
        ...
        {
            name: "key",
            description: Translation.translate("Set API key"),
            execute: args => {
                if (args[0] == "get") {
                    Ai.printApiKey();
                } else {
        ...
        {
            name: "save",
            description: Translation.translate("Save chat"),
            execute: args => {
                const joinedArgs = args.join(" ");
                if (joinedArgs.trim().length == 0) {
                    Ai.addMessage(Translation.translate("Usage: %1save CHAT_NAME").arg(root.commandPrefix), Ai.interfaceRole);
                    return;
                }
        ...
        {
            name: "load",
            description: Translation.translate("Load chat"),
            execute: args => {
                const joinedArgs = args.join(" ");
                if (joinedArgs.trim().length == 0) {
                    Ai.addMessage(Translation.translate("Usage: %1load CHAT_NAME").arg(root.commandPrefix), Ai.interfaceRole);
                    return;
                }
        ...
        {
            name: "clear",
            description: Translation.translate("Clear chat history"),
            execute: () => {
                Ai.clearMessages();
            }
        },
        {
            name: "temp",
            description: Translation.translate("Set temperature (randomness) of the model. Values range between 0 to 2 for Gemini, 0 to 1 for other models. Default is 0.5."),
            execute: args => {
                // console.log(args)
                if (args.length == 0 || args[0] == "get") {
                    Ai.printTemperature();
        ...
        {
            name: "test",
            description: Translation.translate("Markdown test"),
            execute: () => {
                Ai.addMessage(`
<think>
    ...
    function handleInput(inputText) {
        if (inputText.startsWith(root.commandPrefix)) {
            ...
            if (commandObj) {
                ...
                Ai.addMessage(Translation.translate("Unknown command: ") + command, Ai.interfaceRole);
            }
        } else {
    ...
    ColumnLayout {
        ...
        Item {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    StatusItem {
                        ...
                        statusText: ""
                        description: Ai.currentModelHasApiKey ? Translation.translate("API key is set\nChange with /key YOUR_API_KEY") : Translation.translate("No API key\nSet it with /key YOUR_API_KEY")
                    }
                    StatusSeparator {}
                    StatusItem {
                        icon: "device_thermostat"
                        statusText: Ai.temperature.toFixed(1)
                        description: Translation.translate("Temperature\nChange with /temp VALUE")
                    }
                    StatusSeparator {
            ...
            PagePlaceholder {
                ...
                icon: "neurology"
                title: Translation.translate("Large language models")
                description: Translation.translate("Type /key to get started with online models\nCtrl+O to expand sidebar\nCtrl+P to pin sidebar\nCtrl+D to detach sidebar")
                ...
                ScrollView {
                    ...
                    StyledTextArea { // The actual TextArea (inside ScrollView to enable scrolling)
                        ...
                        onActiveFocusChanged: {
                            if (root.scopeRoot) {
                                // ponytail: request stronger layer-shell keyboard focus
                                // only while the AI text input is active, so Fcitx
                                // can commit Chinese preedit text into this TextArea.
                                root.scopeRoot.keyboardFocusExclusive = activeFocus;
                            }
                        }
                            ...
                            if (messageInputField.text.length === 0) {
                                ...
                                root.suggestionList = promptFileResults.map(file => {
                                    return {
                                        ...
                                        description: Translation.translate("Load prompt from %1").arg(file.target)
                                    };
                                });
                                ...
                                root.suggestionList = promptFileResults.map(file => {
                                    ...
                                    return {
                                        ...
                                        description: Translation.translate("Save chat to %1").arg(chatName)
                                    };
                                });
                                ...
                                root.suggestionList = promptFileResults.map(file => {
                                    ...
                                    return {
                                        ...
                                        displayName: `${chatName}`,
                                        description: Translation.translate(`Load chat from %1`).arg(file.target)
                                    };
                                });
